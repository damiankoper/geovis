...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{diagrams/out/c3_scene_graph.png}
...
Komponent Silnika zaimplementowany został w~języku TypeScript\cite{TypeScript}. Jest on nadzbiorem języka JavaScript i~umożliwia korzystanie ze statycznego typowania, które w~JavaScripcie nie jest możliwe. Udostępnia tworzenie unii typów, zawiera mechanizmy ich wnioskowania. Wprowadza wzorzec dekoratorów, zawiera obsługę formatu JSX i~dodaje zmienne wyliczeniowe. Przed uruchomieniem musi być transpilowany do języka JavaScript. Walidacja utworzonego kodu pod względem poprawności typowania następuje w~momencie transpilacji i~nie jest dokonywana podczas wykonywania kodu. 
...
\textit{GeoVisCore} jest komponentem odpowiedzialnym za dostarczenie elementu Canvas, który następnie osadzony jest w~komponencie \textit{GeoVisCoreVue}. Obsługuje on zdarzenia wygenerowane przez użytkownika pochodzące z~elementu Canvas oraz realizuje on cykl życia wizualizacji, wyświetlając opisane przez nią obiekty i~przekazując do niej zdarzenia pochodzące od użytkownika. To w~tym komponencie osadzone jest środowisko biblioteki Three.js.
...
Klasa \texttt{GeoVisCore}, której zależności przedstawia diagram \ref{fig:c3_geo_vis_core}, jest głównym elementem komponentu Silnika. Inicjalizuje one zależności ekosystemu biblioteki Three.js, odpowiada za obsługę elementu Canvas oraz za uruchamianie i~aktualizowanie wizualizacji.

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{diagrams/out/c3_geo_vis_core.png}
    \caption{Diagram klas dla klasy \texttt{GeoVisCore}}
    \label{fig:c3_geo_vis_core}
\end{figure}

Najważniejszą klasą, odpowiedzialną za sterowanie kamerą, jest klasa \texttt{TrackballController}. Implementuje ona interfejs \texttt{TrackballCamera}, który definiuje metody dostępne dla twórcy wizualizacji. W języku TypeScript interfejs może również definiować atrybuty obiektu. Interfejs \texttt{TrackballCamera} definiuje atrybuty zawierające obiekty z~interfejsem \texttt{IEvent}, które służą do definiowania procedur obsługi zdarzeń. Wywołuje je klasa \texttt{TrackballController}. Mechanizm obsługi zdarzeń w~języku JavaScript identyfikują zdarzenie jako ciąg znaków. Aby wykorzystać statyczne typowanie, które umożliwia TypeScript, do obsługi zdarzeń użyto biblioteki \texttt{Strongly Typed Events} \cite{Events}.

\texttt{TrackballController} realizuje operacje obrotów orbit opisane w~rozdziale \ref{sec:camera}. Orbity reprezentowane są przez obiekty \texttt{GlobalOrbit} i~\texttt{LocalOrbit} dziedziczącące po klasie \texttt{Orbit}, które realizuje wspólne operacja dla obu z nich. Są to między innymi wykonywanie obrotów korekcyjnych dla pozycji kamery i~orientacji w~kierunku północnym. Wspólną funkcjonalność różni układ odniesienia obu orbit i~dlatego wydzielono metody abstrakcyjne klasy \texttt{Orbit}, które jej klasy pochodne implementują inaczej. Klasa \texttt{AnimatedTransition} reprezentuje animację pojedynczego obiektu. Zawiera ona swój własny zegar i~pozwala zdefiniować długość oraz funkcję wygładzającą. Diagram najważniejszych zależności klasy \texttt{TrackballController} pokazano na rysunku \ref{fig:c3_trackball}.
